...
abstract class AbstractRequest extends \Omnipay\Common\Message\AbstractRequest
{
    ...
    /**
     * Montant converti en centimes entiers (requis par l'API EasyTransac).
     */
    ...
    public function getUserAgent(): ?string
    {
        return $this->getParameter('userAgent')
            ?: ($this->httpRequest->server->get('HTTP_USER_AGENT') ?: null);
    }
    ...
    /**
     * Ajoute les options communes à tous les types de paiement :
     * - CancelUrl, Language, ReturnMethod, OperationType
     * - SaveCard (pour activer l'enregistrement carte → one-click ultérieur)
     * - UserAgent (requis pour 3DS)
     * - MultiplePayments / MultiplePaymentsRepeat (paiement en N fois)
     * - Rebill + Recurrence (abonnements)
     * - PreAuth / PreAuthDuration (pré-autorisation)
     * - PayToEmail / PayToId (marketplace)
     * - DownPayment (acompte)
     */
    ...
    protected function buildCustomerDataFromCard(): array
    {
        ...
        // Uid marchand (identifiant client côté marchand)
        $uid = $this->getParameter('uid');
        if ($uid !== null && $uid !== '') {
            $customer['Uid'] = $uid;
        }

        return array_filter($customer, static fn($value) => $value !== null && $value !== '');
    }
    ...
    /**
     * Convertit un code ISO2 pays en ISO3 (requis par l'API EasyTransac).
     */
    protected function convertCountryToIso3(?string $country): string
    {
        ...
        $map = [
            'FR' => 'FRA', 'BE' => 'BEL', 'CH' => 'CHE', 'LU' => 'LUX',
            'DE' => 'DEU', 'ES' => 'ESP', 'IT' => 'ITA', 'NL' => 'NLD',
            'PT' => 'PRT', 'GB' => 'GBR', 'IE' => 'IRL', 'US' => 'USA',
            'CA' => 'CAN', 'MA' => 'MAR', 'DZ' => 'DZA', 'TN' => 'TUN',
            'SN' => 'SEN', 'CI' => 'CIV', 'CM' => 'CMR', 'MG' => 'MDG',
            'RE' => 'REU', 'YT' => 'MYT', 'GP' => 'GLP', 'MQ' => 'MTQ',
            'GF' => 'GUF', 'MC' => 'MCO',
        ];
